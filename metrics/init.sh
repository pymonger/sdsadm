...
for i in $(ls metrics/config/conf.d); do
  sed "s/__IPADDRESS_ETH0__/$IPADDRESS_ETH0/g" metrics/config/conf.d/$i | \
...
sudo chmod -R 1777 $comp_dir/log/elasticsearch || :
sudo chown -R 1000 $comp_dir/log/elasticsearch || :
sudo chmod -R 1777 $comp_dir/var/lib/elasticsearch || :
